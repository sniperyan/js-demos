...
因此，我不推荐使用`immutable.js`。设计好组件结构，巧妙使用浅比较，同样也能使RN里的长列表组件或者其他组件获得很好的性能，而且浅比较的速度更快。
...
实际上列表中的数据发生变化，是开发者暴露出的入口给用户去操作，才能改变页面，所以开发者完全是知道场景和state改变的对应关系，这正如伟大的redux所提倡的纯函数的函数式编程的思想一样，所有的变化都是可期的。所以我们优化了列表中的每一个item之后，列表的性能自然会提升，因为列表就是由每一个Item构成的，这些item才是直接操作dom的最小单位。